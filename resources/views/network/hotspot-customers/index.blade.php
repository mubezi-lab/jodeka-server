...
<div class="hc">
 ...
 <div class="panel pad">
  <h2>Send Manual SMS</h2><p class="muted">Chagua customer aliyepo au sajili jina na namba mpya. Namba moja haiwezi kusajiliwa mara mbili.</p>
  <form method="POST" action="{{ route('hotspot-customers.manual-sms.store') }}" id="manualSmsForm" class="manual">@csrf
   <select class="field" id="savedContact"><option value="">New customer / type manually</option>@foreach($contactOptions as $contact)<option data-name="{{ $contact->name }}" data-phone="{{ $contact->normalized_phone }}">{{ $contact->name ?: 'No name' }} — {{ $contact->normalized_phone }}</option>@endforeach</select>
   <input class="field" id="customerName" name="name" value="{{ old('name') }}" placeholder="Customer name" required>
   <input class="field" id="customerPhone" name="phone" value="{{ old('phone') }}" placeholder="0659840000" required>
