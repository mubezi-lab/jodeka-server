...
<div id="request-modal" class="hidden fixed inset-0 z-50 bg-black/50 p-4 overflow-y-auto">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl mx-auto my-6 p-6">
        <div class="flex justify-between items-center mb-5"><div><h3 class="font-bold text-xl">New Stock Request</h3><p id="request-progress" class="text-sm text-gray-500">Request details</p></div><button type="button" id="close-request-modal" class="text-2xl text-gray-500">&times;</button></div>
        <form id="stock-request-form" method="POST" action="{{ route('procurement.requests.store') }}">@csrf
            <section id="request-entry-step">
                @if($fixedBusiness)
                    <div class="bg-gray-50 border rounded p-3 mb-3"><span class="text-gray-500 text-sm">Branch</span><strong class="block">{{ $fixedBusiness->name }}</strong></div><input type="hidden" name="business_id" value="{{ $fixedBusiness->id }}">
                @else
                    <label class="block mb-1">Branch *</label><select name="business_id" class="w-full border rounded p-2 mb-3" required><option value="">Select branch</option>@foreach($businesses as $business)<option value="{{ $business->id }}">{{ $business->name }}</option>@endforeach</select>
                @endif
                @if($canBackdate)
                    <label class="block mb-1">Request Date *</label><input type="date" name="request_date" value="{{ date('Y-m-d') }}" max="{{ date('Y-m-d') }}" class="w-full border rounded p-2 mb-4" required>
                @else
                    <div class="bg-gray-50 border rounded p-3 mb-4"><span class="text-gray-500 text-sm">Request Date</span><strong class="block">{{ now()->format('d/m/Y') }}</strong></div>
                @endif
                <div class="border rounded p-3 mb-4 bg-gray-50">
                    <label class="block mb-1">Product *</label><select id="product-picker" class="w-full border rounded p-2 mb-2"><option value="">Select product</option>@foreach($products as $product)<option value="{{ $product->id }}">{{ $product->name }} ({{ $product->package_type }})</option>@endforeach</select>
                    <label class="block mb-1">Packages *</label><input id="quantity-picker" class="w-full border rounded p-2 mb-2" type="number" min="0.01" step="0.01" placeholder="Packages">
                    <label class="block mb-1">Item Notes</label><input id="notes-picker" class="w-full border rounded p-2 mb-3" placeholder="Optional notes">
                    <button type="button" id="add-item" class="w-full bg-blue-600 text-white rounded p-2">Add Product</button>
                </div>
                <div id="selected-wrapper" class="hidden mb-4"><div class="flex justify-between mb-2"><h4 class="font-semibold">Selected Products</h4><span id="selected-count" class="text-sm text-gray-500"></span></div><div id="request-items" class="space-y-2 max-h-56 overflow-y-auto pr-1"></div></div>
                <label class="block mb-1">General Notes</label><textarea name="notes" class="w-full border rounded p-2" placeholder="Optional notes"></textarea>
            </section>
            <section id="request-summary-step" class="hidden"><h4 class="font-bold text-lg mb-3">Confirm Stock Request</h4><div id="request-summary" class="border rounded divide-y max-h-[55vh] overflow-y-auto"></div></section>
            <p id="item-error" class="hidden text-red-600 text-sm mt-3"></p>
            <div class="flex justify-between mt-6"><button type="button" id="request-back" class="hidden bg-gray-200 px-4 py-2 rounded">Back</button><button type="button" id="request-review" class="ml-auto bg-blue-600 text-white px-4 py-2 rounded">Review Request</button><button type="submit" id="request-submit" class="hidden ml-auto bg-green-600 text-white px-4 py-2 rounded">Submit Request</button></div>
        </form>
    </div>
</div>
